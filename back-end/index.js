...
// Arquivo do Resource (onde estão as rotas)

adminRouter.post('/livros', async (req, res) => {
    ...
    if (dadosLivro.id !== undefined) {
        delete dadosLivro.id;
    }

    const idBruto = await livrosService.getNewId(livros); 
    const novoId = Number(idBruto); 
    
    if (isNaN(novoId)) {
        return res.status(500).json({ mensagem: 'Falha ao gerar ID do livro. ID inválido.' });
    }
